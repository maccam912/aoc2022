...
fn testBelow(stack: []u8, block_type: u8, x_coord: i8, y_coord: usize) bool {
    if (y_coord == 0) {
        return false;
    }
    if (block_type == 0) {
        var block_layer: u8 = 0b1111 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
        if ((block_layer & stack[y_coord - 1]) == 0) {
        ...
        if ((block_layer_0 & stack[y_coord - 1]) == 0 and (block_layer_1 & stack[y_coord]) == 0 and (block_layer_2 & stack[y_coord + 1]) == 0) {
            // No overlap below! We can go down.
            return true;
        } else {
            return false;
        ...
        if ((block_layer_0 & stack[y_coord - 1]) == 0 and (block_layer_1 & stack[y_coord]) == 0 and (block_layer_2 & stack[y_coord + 1]) == 0 and (block_layer_3 & stack[y_coord + 2]) == 0) {
            // No overlap below! We can go down.
            return true;
        } else {
            return false;
        ...
        if ((block_layer_0 & stack[y_coord - 1]) == 0 and (block_layer_1 & stack[y_coord]) == 0) {
            // No overlap below! We can go down.
            return true;
        } else {
            return false;
...
fn checkLateral(stack: []u8, block_type: u8, x_coord: i8, y_coord: usize, instruction: i8) bool {
    ...
    if (block_type == 0) {
        ...
        if ((block_layer & stack[y_coord]) == 0) {
    ...
    if (block_type == 1) {
        ...
        if ((block_layer_0 & stack[y_coord]) == 0 and (block_layer_1 & stack[y_coord + 1]) == 0 and (block_layer_2 & stack[y_coord + 2]) == 0) {
            // It can move without overlap
            return true;
        } else {
            return false;
        ...
        if ((block_layer_0 & stack[y_coord]) == 0 and (block_layer_1 & stack[y_coord + 1]) == 0 and (block_layer_2 & stack[y_coord + 2]) == 0 and (block_layer_3 & stack[y_coord + 3]) == 0) {
            // It can move without overlap
            return true;
        } else {
            return false;
        ...
        if ((block_layer_0 & stack[y_coord]) == 0 and (block_layer_1 & stack[y_coord + 1]) == 0) {
            // It can move without overlap
            return true;
        } else {
            return false;
...
fn updateStack(stack: []u8, block_type: u8, x_coord: i8, y_coord: usize) usize {
    if (block_type == 0) {
        var block_layer: u8 = 0b1111 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
        stack[y_coord] = block_layer | stack[y_coord];
        return y_coord;
    } else if (block_type == 1) {
        var block_layer_2: u8 = 0b010 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
        var block_layer_1: u8 = 0b111 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
        var block_layer_0: u8 = 0b010 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
        stack[y_coord] = block_layer_0 | stack[y_coord];
        stack[y_coord + 1] = block_layer_1 | stack[y_coord + 1];
        stack[y_coord + 2] = block_layer_2 | stack[y_coord + 2];
        return y_coord + 2;
    } else if (block_type == 2) {
        var block_layer_2: u8 = 0b001 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
        var block_layer_1: u8 = 0b001 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
        ...
        stack[y_coord + 1] = block_layer_1 | stack[y_coord + 1];
        stack[y_coord + 2] = block_layer_2 | stack[y_coord + 2];
        stack[y_coord + 3] = block_layer_3 | stack[y_coord + 3];
        return y_coord + 3;
    } else if (block_type == 4) {
        var block_layer_1: u8 = 0b11 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
        var block_layer_0: u8 = 0b11 * std.math.pow(u8, 2, @intCast(u8, (7 - blockWidth(block_type)) - x_coord));
...
fn stackHeight(stack: []u8, start: usize) usize {
...
pub fn partA(allocator: std.mem.Allocator) !usize {
    ...
    var stack: [2022 * 4]u8 = std.mem.zeroes([2022 * 4]u8);
    ...
    while (block_count < 3160) {
        ...
        while (testBelow(stack[0..], block_type, x_coord, y_coord)) {
            y_coord -= 1;
        ...
        // update stack
        _ = updateStack(&stack, block_type, x_coord, y_coord);
...
pub fn partB(allocator: std.mem.Allocator) !usize {
    _ = allocator;
    const input = comptime inputText();
    const count = input.len * 5 * 3 * 5;
    var stack: [count * 2]u8 = std.mem.zeroes([count * 2]u8);
    var block_count: usize = 0;
    var step: usize = 0;
    var stack_top: usize = 0;
        ...
        if (block_count > input.len * 15) {
            ...
            while (offset < input.len * 5 * 4) : (offset += 1) {
                ...
                while (block_length < input.len * 5 * 4) : (block_length += 1) {
                    var lower = stack[offset .. offset + block_length];
                    var upper = stack[offset + block_length .. offset + block_length * 2];
